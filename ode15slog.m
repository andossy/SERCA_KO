function varargout = ode15slog(ode,tspan,y0,options,varargin)
...
%   $Revision: 1.83 $  $Date: 2002/04/08 20:26:46 $
...
[neq, tspan, ntspan, next, t0, tfinal, tdir, y0, f0, args, ...
         ...
         = odearguments(FcnHandlesUsed,'ode15s', ode, tspan, y0, options,  ...
...
% Initialize logging Hack by Hake 08.2005
if nargin-5 == 1 && nargout == 3 && length(args{2}) > 0
  logging = 1;
  loginfo = args{2};
...
if nargout > 0
  if ntspan > 2                         % output only at tspan points
    ...
    if soloutRequested
      chunk = min(max(100,50*refine),floor((2^11)/neq));
    else          
    ...
    if logging
      for i=1:nlogvar 
        logout.(logvar{i}) = zeros(chunk,loginfo.(logvar{i}));
      end
    end
    if soloutRequested
      kvec = zeros(chunk,1);
      dif3d = zeros(chunk,neq,maxk+2);
    end
  end
  nout = 1;
  tout(nout) = t;
...
% Initialize logging Hack by Hake 08.2005
if logging
  [f0 logres] = feval(ode,t,y,args{:});
  for i=1:nlogvar
    logout.(logvar{i})(nout,:)=logres{i};
  end
end
% THE MAIN LOOP
        ...
        elseif absh <= hmin
          ...
          if nargout > 0
            if soloutRequested
              ...
              sol.idata.dif3d = dif3d(1:nout,:,1:maxkvec+2);
              if haveeventfun
                sol.xe = teout.';
                sol.ye = yeout.';
                sol.ie = ieout.';
  ...
  if nargout > 0
    ...
    if outflag == 2                     % computed points, no refinement
      ...
      if nout > length(tout)
        ...
        if soloutRequested
          kvec = [kvec; zeros(chunk,1)];
          dif3d = [dif3d; zeros(chunk,neq,maxk+2)];
        end
      end
      tout(nout) = tnew;
      yout(nout,:) = ynew.';
  ...
  if soloutRequested
    ...
    if haveeventfun
      ...
      if ~FcnHandlesUsed
        ...
        if logging %hack
          for i=1:nlogvar
            logout.(logvar{i}) = logout.(logvar{i})(1:nout,:);
          end
          varargout{3} = logout;
        end
      end
